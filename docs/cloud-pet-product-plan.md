...
- Community comment authors can now edit their own visible comments from the stable detail page; comment identity, thread associations, withdrawal semantics, and parent-post visibility remain intact.
...
- Phase 7 production startup validation now blocks missing database configuration, memory-store mode, default or weak admin keys, local CORS origins, and invalid payment timeouts. Owner email and password are bootstrap-only inputs and are not required by the long-running API after an active Owner Staff exists.
...
- Phase 7 cloud-pet operations now includes an isolated fresh-database handover drill that runs migration, one-time Owner bootstrap, runtime secret removal, production readiness, real Owner/member browser flows, and final Prisma evidence without touching the development database.
- Phase 7 SQLite deployment hardening now enforces one active Nest runtime per normalized production SQLite database path, rejects a second runtime before traffic, releases ownership on shutdown, recovers only dead-process stale records, and verifies same-database conflict plus different-database isolation through a focused process smoke.
- Phase 7 planned-restart hardening now verifies existing Owner/member/cloud-pet data across a graceful same-database Nest runtime handover, with bootstrap secrets removed from the replacement runtime and no extra care diary created by the restart.
- Phase 7 production host preflight now checks the real SQLite path, data-directory probe, runtime ownership acquire/release, API/Web ports, production configuration, release marker, built artifacts, and read-only migration compatibility without starting business services or writing business data.
...
- Admin 云养宠的非敏感结构化筛选与排序现在可通过 URL 恢复；自由文本搜索不写入 URL，刷新和历史导航继续复用现有筛选管线。
- Admin 云养宠运营区现在支持手动刷新列表与当前详情，保留已应用的搜索和结构化筛选，并显示最近一次成功更新的本地时间；刷新失败不会清空既有数据。
...
- Community post details now expose stable same-origin comment and reply links using commentNo fragments; valid targets scroll into view with a temporary focus state, while invalid or withdrawn targets remain a normal detail page without expanding data access.
- Admin community moderation now reuses the existing CommunityComment status field to independently hide a reported comment, update the report queue, preserve the parent post and author-withdrawal semantics, and record a staff-bound operation log; no new schema or moderation state machine was added.
- Phase 7 production browser smoke now covers the real comment-report moderation loop: a member creates and reports a comment, an Owner hides only that comment from Admin, the parent post remains visible while the comment deep-link does not leak it, and Prisma verifies the comment/report states plus staff-bound operation logs.
- Cloud-pet MVP launch acceptance now has one fail-closed `npm run verify:cloud-pet-launch` entry that serially orchestrates the existing unit, API E2E, single-worker Playwright, build, production smoke, and diff checks without adding another business or production harness.
